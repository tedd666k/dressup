...
export default function Checkout() {
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      <p className="text-sm text-muted-foreground mb-6">Ghana Mobile Money supported. Orders send to WhatsApp.</p>
      ...
      {settings.momoProvider === "manual" ? (
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          ...
          <Button variant="outline" disabled className="cursor-not-allowed">Pay with Mobile Money (connect {settings.momoProvider})</Button>
        </div>
      )}
...
function ManualMomo({ orderText, ownerPhone }: { orderText: string; ownerPhone?: string }) {
  const [phone, setPhone] = React.useState("");
  const [network, setNetwork] = React.useState("MTN");
  const link = buildWhatsAppOrder({ phone: ownerPhone, message: `${orderText}MoMo request — Network: ${network}, Customer MoMo: ${phone}` });
  return (
    <div className="mt-6 rounded-xl border p-4">
      <h3 className="font-medium mb-3">Mobile Money (Manual)</h3>
      <div className="grid md:grid-cols-3 gap-3">
        <input className="h-10 rounded-md border bg-background px-3" placeholder="Customer MoMo Number" value={phone} onChange={(e)=>setPhone(e.target.value)} />
        <select className="h-10 rounded-md border bg-background px-3" value={network} onChange={(e)=>setNetwork(e.target.value)}>
          <option>MTN</option>
          <option>Vodafone</option>
          <option>AirtelTigo</option>
        </select>
        <Button disabled={!link} onClick={()=> link && window.open(link, "_blank")}>Send Payment Request via WhatsApp</Button>
      </div>
      {!ownerPhone && <p className="mt-2 text-xs text-muted-foreground">Set owner WhatsApp in Admin → Settings.</p>}
    </div>
  );
}
